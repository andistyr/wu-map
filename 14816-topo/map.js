var change_map, clear_home, close_infowin, deed_tags, distance, filter, find_nearby_locations, hide_add_form, hide_search, infowin, init, marker, projection, search, set_home, share_coords, share_deed, show_add_form, show_coords_info, show_coords_on_map, show_deed_info, show_deed_on_map, toggle_markers, toggle_serverinfo_size, toggle_sidebar, update_markers, update_stats, vote_reminder_close, vote_reminder_open,
  indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] == item) return i; } return -1; };
    ...
    var TILE_URL = 'https://web.game.sklotopolis.com/unlimited/2/tiles/tile_{z}_{x}_{y}.png';
...
init = function() {
  ...
  Sklotopolis = new google.maps.ImageMapType({
    getTileUrl: function(coord, zoom) {
      if (coord.x == 0 && coord.y == 0) {
        return 'https://web.game.sklotopolis.com/unlimited/2/mapdump.png';
      }
    },
  ...
  if (hash.indexOf('_') !== -1) {
    ...
    home_deed = localStorage.getItem('wu_map_home_deed_14816_topo');
    if (home_deed != null) {
      if (home_deed !== '') {
...
set_home = function(tag, img) {
  localStorage.setItem('wu_map_home_deed_14816_topo', tag);
  return show_deed_info(tag);
};

clear_home = function() {
  return localStorage.setItem('wu_map_home_deed_14816_topo', '');
};
...
show_deed_info = function(tag) {
  ...
  if (localStorage.getItem('wu_map_home_deed_14816_topo') == deed.tag) {
...
share_deed = function(tag, el) {
  ...
  el.style.padding = 0;
  el.innerHTML = '<input type="text" value="https://andistyr.github.io/wu-map/14816-topo/#' + tag + '" style="width:280px;padding:2px;border-radius:3px;border:1px solid #dedede;font-size:12px" onclick="this.select()" />';
...
share_coords = function(x, y, el) {
  el.style.backgroundColor = 'white';
  el.style.padding = 0;
  el.innerHTML = '<input type="text" value="https://andistyr.github.io/wu-map/14816-topo/#' + x + '_' + y + '" style="width:255px;padding:2px;border-radius:3px;border:1px solid #dedede;font-size:12px" onclick="this.select()" />';
  el.childNodes[0].select();
  return false;
};
