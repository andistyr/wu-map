...
init = function() {
  ...
  if (hash.indexOf('_') !== -1) {
    ...
    home_deed = localStorage.getItem('wu_map_home_deed_14851_topo');
    if (home_deed != null) {
      if (home_deed !== '') {
...
set_home = function(tag, img) {
  localStorage.setItem('wu_map_home_deed_14851_topo', tag);
  return show_deed_info(tag);
};

clear_home = function() {
  return localStorage.setItem('wu_map_home_deed_14851_topo', '');
};
...
show_deed_info = function(tag) {
  ...
  if (localStorage.getItem('wu_map_home_deed_14851_topo') == deed.tag) {
...
share_deed = function(tag, el) {
  ...
  el.style.padding = 0;
  el.innerHTML = '<input type="text" value="https://andistyr.github.io/wu-map/14851-topo/#' + tag + '" style="width:280px;padding:2px;border-radius:3px;border:1px solid #dedede;font-size:12px" onclick="this.select()" />';
...
share_coords = function(x, y, el) {
  el.style.backgroundColor = 'white';
  el.style.padding = 0;
  el.innerHTML = '<input type="text" value="https://andistyr.github.io/wu-map/14851-topo/#' + x + '_' + y + '" style="width:255px;padding:2px;border-radius:3px;border:1px solid #dedede;font-size:12px" onclick="this.select()" />';
  el.childNodes[0].select();
  return false;
};
