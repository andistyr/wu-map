...
init = function() {
  ...
  var swBound = projection.fromPointToLatLng(projection.fromCoords({ x: 0, y: 2048 }));
  var neBound = projection.fromPointToLatLng(projection.fromCoords({ x: 2048, y: 0 }));
...
clear_home = function(tag) {
  localStorage.setItem('wu_map_home_deed_14902', '');
  return show_deed_info(tag);
};
...
share_deed = function(tag, el) {
  const url = `https://andistyr.github.io/wu-map/14902-topo/#${tag}`;
...
share_coords = function(x, y, el) {
  const url = `https://andistyr.github.io/wu-map/14902-topo/#${x}_${y}`;
